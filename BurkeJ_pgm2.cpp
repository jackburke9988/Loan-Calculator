...
// Author: Jack Burke
//
// Reflection: Ensuring numeric accuracy and robust input handling
//             was the hardest part, especially avoiding rounding errors
//             in the calculation. Input validation for mixed user input
//             (months vs. years) also was tricky at first.
//
//             Next time I would split the program into small functions and perform
//             math in integer cents or with higher precision  to
//             get rid of rounding and make testing easier.
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
